...
void printResetReason() {
  ...
  Serial.printf("Reset reason: %d\n", r); // 1=POWERON, 2=EXT, 3=SW, 4=PANIC, 5=INT_WDT, 6=TASK_WDT, 7=RTC_WDT, 8=BROWNOUT, etc.
}
...
const double AutoPumpOnMaxTm = 120000;            // milliseconds
const double AutoPumpOnMinTm = 10000;             // milliseconds
const double AutoSolOnMaxTm = 120000;        // milliseconds
const double AutoSolOnMinTm = 1000;          // milliseconds
const double ManualPumpOnMinTm = 0.00;             // millieconds
const double ManualSolOnMinTm = 0.00;          // milliSeconds
const int InflatePin = 16;
const int DeflatePin = 33;
const unsigned long RefreshRate = 250; // milliseconds
...
void handleRoot() {
    String page = R"rawliteral(
        ...
        <main>
            ...
            <button id="GOBtn" class="btn">STOP / GO</button>
        ...
        function applyBackendButtonStates(word) {
            ...
            goBtn.classList.toggle('active', GOResponse);
            goBtn.textContent = GOResponse ? 'STOP' : 'GO';
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (inflateBtn && deflateBtn) {
                inflateBtn.addEventListener('touchstart', (e) => {
                    e.preventDefault();
                    let requestedStates = { ...uiState, ManualInflateBtn: true };
                    requestStateChange(requestedStates);
                });
                inflateBtn.addEventListener('touchend', (e) => {
                    e.preventDefault()
                    let requestedStates = { ...uiState, ManualInflateBtn: false };
                    requestStateChange(requestedStates);
                });
                deflateBtn.addEventListener('touchstart', (e) => {
                    e.preventDefault();
                    let requestedStates = { ...uiState, ManualDeflateBtn: true };
                    requestStateChange(requestedStates);
                });
                deflateBtn.addEventListener('touchend', (e) => {
                    e.preventDefault()
...
double DeflBounds(double x)
{
    if (x > AutoSolOnMaxTm)
    {
        return AutoSolOnMaxTm;
    }
    else if (x < AutoSolOnMinTm)
    {
        return AutoSolOnMinTm;
    }
    else
    {
        return x;
...
void handleHMIButtonRequest()
{
    ...
    if (server.hasArg("HMIButtonRequestWord"))
    {
        ...
        if (!ReqAUTO && !ReqMANUAL)
        { // Neither AUTO nor MANUAL requested
            Escape();
        }

        if (MANUAL)
        { // If already in MANUAL mode, only switch to AUTO if requested
            if (ReqAUTO)
            {
                AUTO = true;
                MANUAL = false;
        ...
        { // If already in AUTO mode, only switch to MANUAL if requested
            if (ReqMANUAL)
            {
                MANUAL = true;
                AUTO = false;
        ...
        { // If in neither mode, accept either request
            if (ReqAUTO)
            {
                AUTO = true;
                MANUAL = false;
...
void handlePresSetpoint()
{
    if (server.hasArg("PspHMI")) {
        ...
        if ((S0 || ToS0 || S0a || ToS0a || S2 || ToS2) && !GoButtonActive) {
...
void loop()
{
    ...
    if (millis() - lastSerialMonitorOutput >= RefreshRate) {
    ...
    if (S0a || ToS0a)
    {
        ...
        StateMessage = "S0a | Ready to operate";
    ...
    if (S1ab || ToS1ab)
    {
        ...
        oss << "S1ab | Calibration Inflation | " << std::fixed << std::setprecision(2) << ((CalibrationTm - (millis() - S1abSttTm)) / 1000.0) << " seconds remaining";
    ...
    if (S1ae || ToS1ae)
    {
        ...
        // Calcualte inflation time (milliseconds)
        m = (Pc2 - Pc1)/DeltaTm;
    ...
    if (S1af || ToS1af)
    {
        ...
        // Update state variable message
        std::ostringstream oss;
        oss << "S1af | Smart Inflation | " << std::fixed << std::setprecision(2) << (InflTm - (millis() - S1afSttTm)) / 1000 << " seconds remaining";
        StateMessage = oss.str();

        // Transition onward code
        if (S1afSttTm != 0.00 && (millis() - S1afSttTm >= InflTm))
        {
            PumpON = false;
            InflTm = 0.00;
            S1afSttTm = 0.00;
    ...
    if (S1bb || ToS1bb)
    {
        ...
        // Update state variable message
        std::ostringstream oss;
        oss << "S1bb | Calibration Deflation | " << std::fixed << std::setprecision(2) << ((CalibrationTm - (millis() - S1bbSttTm)) / 1000) << " seconds remaining";
        StateMessage = oss.str();
    ...
    if (S1be || ToS1be)
    {
        ...
        // Calcualte deflation time (milliseconds)
    ...
    if (S1bf || ToS1bf)
    {
        ...
        // Update state variable message
        std::ostringstream oss;
        oss << "S1bf | Smart Deflation | " << std::fixed << std::setprecision(2) << ((DeflTm - (millis() - S1bfSttTm)) / 1000) << " seconds remaining";
        StateMessage = oss.str();

        // Transition onward code
        if (S1bfSttTm != 0.00 && (millis() - S1bfSttTm >= DeflTm))
        {
            SolenoidON = false;
            DeflTm = 0.00;
            S1bfSttTm = 0.00;
    ...
    // S1cx | Dumb deflation | ------------------------------------------------------------------------------------------------
    // S1c | Deflate for AutoSolOnMinTm | multi-frame | -----------------------------------------------------------------------------------
    if (S1c || ToS1c)
    {
        // Escape condition
        if (!GoButtonActive || !AUTO)
        ...
        // Update state variable message        
        std::ostringstream oss;
        oss << "S1c | Dumb Deflation | " << std::fixed << std::setprecision(2) << ((AutoSolOnMinTm - (millis() - S1cSttTm)) / 1000) << " seconds remaining";
        StateMessage = oss.str();
        ...
        // Transition onward code
        if (S1cSttTm != 0.00 && (millis() - S1cSttTm >= AutoSolOnMinTm))
        {
            SolenoidON = false;
            S1cSttTm = 0.00;
    ...
    if (S2a || ToS2a)
    {
        ...
        if (ManualInflate && millis() - S2aSttTm >= ManualPumpOnMinTm)
        {
            PumpON = true;
        }
        else if (ManualInflate && millis() - S2aSttTm < ManualPumpOnMinTm)
        {
            PumpON = false;
        }
        else
    ...
    if (S2b || ToS2b)
    {
        ...
        if (ManualDeflate && millis() - S2bSttTm >= ManualSolOnMinTm)
        {
            SolenoidON = true;
        }
        else if (ManualDeflate && millis() - S2bSttTm < ManualSolOnMinTm)
        {
            SolenoidON = false;
        }
        else
